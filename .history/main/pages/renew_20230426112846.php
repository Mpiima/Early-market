...
if($status=="cancelled"){
    ...
    $_SESSION['period'];   
    $amt=1000;
    $amount=$_SESSION['period']*$amt;
    $insert_sub=$dbh->query("INSERT INTO subscription(ref_no,amount,client_id,expiry_date,period,status)
    values('$txtref','$amount','$clientid','$expirydate','".$_SESSION["period"]."',1)");
    if($insert_sub){
        ...
        ?>
        <script>
        var allowed=function(){window.location='mysubscription.php';}
...
if(isset($_POST['myselect'])){
    echo "Your paying for = ". $_POST['myselect'] ." months";
    // echo "Amount = ". $_POST['myselect'] * 4000;
     $_SESSION['period']=$_POST['myselect'];
}
?>
</div>
</div></div>
    ...
    <div class="row">
            <div class="col-lg-3">
                ...
                <?php if(isset($_POST['myselect'])){
                    ?>
                <input type="hidden" name="amount" value="<?php echo $_POST['myselect']*1000; ?>"  class="form-control"/>
                <input type="text" name="amountt" value="<?php echo $_POST['myselect']*1000; ?>" disabled="disabled"  class="form-control"/>
                    <?php
                } ?>
