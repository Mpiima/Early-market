...
if($status=="cancelled"){
    ...
    $amount=2000;
    // INSERT INTO `subscription`(`autoid`, `status`, `ref_no`, `amount`, `client_id`, 
    // `period`, `created_on`, `expiry_date`, `update_on`)
    $insert_sub=$dbh->query("INSERT INTO subscription(ref_no,amount,client_id,expiry_date)
    values('$txtref','$amount','$clientid','$expirydate')");
    if($insert_sub){
