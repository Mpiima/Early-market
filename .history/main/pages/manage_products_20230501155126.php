...
$insert_products=$dbh->query("insert into product(prod_cat,prod_subcat,address,seller_name,prod_details,bargain,prod_price,productid) values('".$catid."','".$subcatid."','".$address."','".$seller_name."','".$prod_details."','".$bargain."','".$prod_price."','".$prodid."')");
if($insert_products){ echo "<div class='alert alert-success'>Success: Product added successfully</div>"; }else { echo "<div class='alert alert-danger'>Failuere: Product not added</div>"; }
foreach ($multiChoice as $key => $value) {
$insert_prod_attributes=$dbh->query("insert into prod_attributes(productid,singleName,singleValue,multiple_choice) values('".$prodid."','".$singleChoiceID[$key]."','".$singleChoice[$key]."','".$value."') ");
...
<tbody>
<?php 
$result_products=$dbh->query("select * from product where seller_name='".$rolenumber."' and autoid>0 order by autoid desc");
